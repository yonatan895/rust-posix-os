...
pub unsafe fn coreutils_main(argc: usize, argv: *const *const u8) -> i32 {
    ...
    let (applet, sub_argv, sub_argc) = if unsafe { strcmp(prog_name, b"coreutils\0".as_ptr()) } == 0
    {
        if argc <= 1 || unsafe { (*argv.add(1)).is_null() } {
            unsafe { print_usage() };
            return 1;
        }
        let app = unsafe { get_basename(*argv.add(1)) };
        (app, unsafe { argv.add(1) }, argc - 1)
    } else {
        (prog_name, argv, argc)
    };
...
pub unsafe fn cmd_echo(argc: usize, argv: *const *const u8) -> i32 {
    for i in 1..argc {
        ...
        if !arg.is_null() {
            ...
            unsafe { write(STDOUT_FILENO, arg, len) };
        }
    }
    unsafe { putchar(b'\n' as i32) };
    0
