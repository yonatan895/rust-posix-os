...
pub unsafe fn execute_pipeline_line(line: *mut u8, execute_cmd_fn: impl Fn(usize, &[*const u8; 16])) {
    ...
    if num_stages == 1 {
        ...
        let orig_in = dup(STDIN_FILENO);
        let orig_out = dup(STDOUT_FILENO);
        let mut ok = true;
        if !stage.redir.stdin_file.is_null() {
            let fd = open(stage.redir.stdin_file, O_RDONLY, 0);
            if fd >= 0 { dup2(fd, STDIN_FILENO); close(fd); }
            else { printf(b"shell: cannot open '%s' for input\n\0".as_ptr(), stage.redir.stdin_file); ok = false; }
        }
        if ok && !stage.redir.stdout_file.is_null() {
            let flags = O_WRONLY | O_CREAT | if stage.redir.stdout_append { O_APPEND } else { O_TRUNC };
            let fd = open(stage.redir.stdout_file, flags, 0o644);
            if fd >= 0 { dup2(fd, STDOUT_FILENO); close(fd); }
            else { printf(b"shell: cannot open '%s' for output\n\0".as_ptr(), stage.redir.stdout_file); ok = false; }
        }
        if ok { execute_cmd_fn(stage.argc, &stage.argv); }
    ...
    for i in 0..(num_stages - 1) {
        if pipe(&mut pipes[i] as *mut [i32; 2]) < 0 {
            puts(b"shell: pipe creation failed\0".as_ptr());
            return;
        }
    }
    ...
    for i in 0..num_stages {
        ...
        if i < num_stages - 1 { dup2(pipes[i][1], STDOUT_FILENO); }
