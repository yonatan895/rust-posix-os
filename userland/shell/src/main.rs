...
static mut HISTORY: [HistoryEntry; MAX_HISTORY] = [HistoryEntry { buf: [0; MAX_CMD_LEN], len: 0 }; MAX_HISTORY];
static mut HISTORY_COUNT: usize = 0;
static mut LAST_PROMPT_LEN: usize = 0;
...
unsafe fn history_add(cmd: &[u8], len: usize) {
    ...
    if HISTORY_COUNT > 0 {
        ...
        if last.len == len && &last.buf[..len] == &cmd[..len] {
            return;
        }
    }
...
impl<'a> LineBuffer<'a> {
    ...
    fn push_cstr(&mut self, s: *const u8) {
        if s.is_null() { return; }
        let mut ptr = s;
        unsafe {
            while *ptr != 0 {
    ...
    fn flush(&self) {
        if self.len > 0 {
            unsafe {
                write(STDOUT_FILENO, self.buf.as_ptr(), self.len);
            }
        }
    }
}
...
unsafe fn repaint_prompt_line(cwd: *const u8, buf: &[u8], len: usize) {
    ...
    // Direct carriage return (no screen blanking)
    out.push_str("\rposix-os:");
    out.push_cstr(cwd);
    out.push_str("# ");

    let prompt_prefix_len = 9 + strlen(cwd) + 2;
    let mut visible_len = prompt_prefix_len;
    ...
    if start < len {
        ...
        while cmd_end < len && buf[cmd_end] != b' ' && buf[cmd_end] != b'\t' && buf[cmd_end] != b'|' && buf[cmd_end] != b'>' && buf[cmd_end] != b'<' {
            cmd_end += 1;
        }

        let cmd_slice = &buf[start..cmd_end];
        let is_valid = if let Ok(s) = core::str::from_utf8(cmd_slice) {
            is_known_command(s)
        } else {
            false
        };
    ...
    // Space-pad trailing difference if new line is shorter
    if visible_len < LAST_PROMPT_LEN {
        let diff = LAST_PROMPT_LEN - visible_len;
        for _ in 0..diff {
            out.push_byte(b' ');
        }
...
unsafe fn handle_tab_completion(cwd: *const u8, buf: &mut [u8], len: &mut usize) {
    ...
    if is_command_name {
        replace_start = start;
        let prefix = if let Ok(s) = core::str::from_utf8(&buf[start..*len]) {
            s
        } else {
            return;
        };
        ...
        for &cmd in KNOWN_COMMANDS.iter() {
            if let Some(sc) = fuzzy_score(prefix, cmd) {
                if match_count < 18 {
                    ...
                    matches[match_count].is_dir = false;
                    match_count += 1;
                }
            }
        }
        ...
        let arg_prefix = if let Ok(s) = core::str::from_utf8(&buf[last_space..*len]) {
            s
        } else {
            return;
        };
...
unsafe fn read_line(cwd: *const u8, buf: &mut [u8]) -> usize {
    ...
    loop {
        ...
        // Append printable character with flicker-free repaint
        if idx < buf.len() - 1 {
            buf[idx] = ch as u8;
            idx += 1;
            repaint_prompt_line(cwd, buf, idx);
...
unsafe fn parse_stage(stage_str: *mut u8) -> Stage {
    ...
    while i < raw_argc {
        ...
        if b0 == b'>' && b1 == b'>' {
            if *token.add(2) != 0 {
                stage.redir.stdout_file = token.add(2);
            ...
            if *token.add(1) != 0 {
                stage.redir.stdout_file = token.add(1);
            } else if i + 1 < raw_argc {
                stage.redir.stdout_file = raw_argv[i + 1];
                i += 1;
            }
            ...
            if *token.add(1) != 0 {
                stage.redir.stdin_file = token.add(1);
            } else if i + 1 < raw_argc {
    ...
    if num_stages == 1 {
        ...
        if !stage.redir.stdin_file.is_null() {
            ...
            if in_fd >= 0 {
                ...
                print_error(b"open\0".as_ptr(), stage.redir.stdin_file, in_fd);
                close(saved_in);
                close(saved_out);
                return;
            }
        ...
        if !stage.redir.stdout_file.is_null() {
            ...
            if out_fd >= 0 {
                ...
                print_error(b"open\0".as_ptr(), stage.redir.stdout_file, out_fd);
                close(saved_in);
                close(saved_out);
                return;
            }
...
unsafe fn execute_command(argc: usize, argv: &[*const u8; 16]) {
    ...
    if strcmp(cmd, b"help\0".as_ptr()) == 0 {
        puts(b"Available POSIX Shell Commands:\n  help                    - Show this command reference\n  uname                   - Display system architecture and version\n  pwd                     - Print working directory\n  cd [dir|-]              - Change directory (supports -, ~, paths)\n  ls [-l|-a|-la] [dir]    - List directory with options (-l long, -a all)\n  cat [-n] [files...]     - Display file(s) or stdin stream (-n numbered)\n  touch [-c] [files...]   - Create files (-c no-create)\n  mkdir [-p] [dirs...]    - Create directories (-p create parent dirs)\n  rm [-r|-f|-rf] [files]  - Remove files or directories (-r recursive, -f force)\n  ps                      - List active processes and states\n  top / monitor           - Interactive system resources and process monitor\n  journal                 - Display security audit event journal\n  snapshot [create|list]  - Manage and inspect system state audit snapshots\n  echo [-n|-e] [args...]  - Print text (-n omit newline, -e interpret escapes)\n  async-demo              - Demonstrate asynchronous tasks & epoll multiplexing\n  clear                   - Clear terminal screen\n  exit                    - Terminate current shell\n\nPipeline & Redirection:\n  cmd1 | cmd2             - Chain stdout of cmd1 to stdin of cmd2\n  cmd > file              - Redirect stdout to file (overwrite/create)\n  cmd >> file             - Redirect stdout to file (append/create)\n  cmd < file              - Redirect stdin from file\0".as_ptr());
...
unsafe fn handle_ls(argc: usize, argv: &[*const u8; 16]) {
    ...
    for p in 0..path_count {
        ...
        list_directory_advanced(path, show_all, long_format, human_readable);
        if path_count > 1 && p + 1 < path_count {
            putchar(b'\n' as i32);
        }
    }
}
...
unsafe fn list_directory_advanced(path: *const u8, show_all: bool, long_format: bool, human: bool) {
    ...
    if fd < 0 {
        ...
        if stat(path, &mut st) == 0 {
            if long_format {
                let type_char = if (st.st_mode & S_IFDIR) != 0 { b'd' } else { b'-' };
                let mode_str = b"rw-r--r--\0".as_ptr();
                printf(b"%c%s  %6d  %s\n\0".as_ptr(), type_char as i32, mode_str, st.st_size as i32, path);
            } else {
                printf(b"  %s\n\0".as_ptr(), path);
            }
            return;
    ...
    loop {
        ...
        while offset < n as usize {
            ...
            let first_char = *name_ptr;

            if !show_all && first_char == b'.' {
                offset += core::mem::size_of::<Dirent64>();
                continue;
            }
                ...
                if path_len + need_slash + name_len < 255 {
                    ...
                    if stat(fullpath.as_ptr(), &mut st) == 0 {
                        ...
                        if human && st.st_size >= 1024 {
                            let kb = (st.st_size + 1023) / 1024;
                            printf(b"%c%s  %4dK  %s%s\n\0".as_ptr(), type_char as i32, mode_str, kb as i32, name_ptr, suffix);
                        } else {
    ...
    for p in 0..path_count {
        ...
        if no_create {
            ...
            if fd >= 0 {
                ...
                print_error(b"touch: cannot create file\0".as_ptr(), path, fd);
            }
        }
    }
}
...
unsafe fn handle_mkdir(argc: usize, argv: &[*const u8; 16]) {
    ...
    for p in 0..path_count {
        ...
        if create_parents {
            ...
            if res < 0 {
                print_error(b"mkdir: cannot create directory\0".as_ptr(), path, res);
            }
        }
    }
}
...
unsafe fn display_journal() {
    let fd = open(b"/proc/audit_journal\0".as_ptr(), O_RDONLY, 0);
    if fd >= 0 {
        puts(b"================================================================================\0".as_ptr());
        puts(b"             Rust POSIX OS - Kernel Security Audit Event Journal                \0".as_ptr());
        puts(b"================================================================================\0".as_ptr());
        let mut buf = [0u8; 4096];
        let n = read(fd, buf.as_mut_ptr(), buf.len() - 1);
        if n > 0 {
            buf[n as usize] = 0;
        ...
        puts(b"journal: unable to read /proc/audit_journal\0".as_ptr());
    }
}

unsafe fn handle_snapshot_command(argc: usize, argv: &[*const u8; 16]) {
    if argc == 1 || (argc > 1 && strcmp(argv[1], b"list\0".as_ptr()) == 0) {
        let fd = open(b"/proc/snapshots\0".as_ptr(), O_RDONLY, 0);
        if fd >= 0 {
            puts(b"================================================================================\0".as_ptr());
            puts(b"               Rust POSIX OS - System State Audit Snapshots                     \0".as_ptr());
            puts(b"================================================================================\0".as_ptr());
            ...
            puts(b"snapshot: unable to read /proc/snapshots\0".as_ptr());
        }
    } else if argc >= 2 && strcmp(argv[1], b"create\0".as_ptr()) == 0 {
        let label = if argc > 2 && !argv[2].is_null() {
            argv[2]
        } else {
            b"manual_checkpoint\0".as_ptr()
        };
        let id = audit_snapshot(label, 0);
        if id > 0 {
            printf(b"[snapshot] Successfully created audit snapshot #%d ('%s')\n\0".as_ptr(), id as i32, label);
        } else {
            printf(b"[snapshot] Failed to create audit snapshot (error %d)\n\0".as_ptr(), id as i32);
        }
    } else {
...
unsafe fn display_ps() {
    let fd = open(b"/proc/processes\0".as_ptr(), O_RDONLY, 0);
    if fd >= 0 {
        let mut buf = [0u8; 2048];
        let n = read(fd, buf.as_mut_ptr(), buf.len() - 1);
        if n > 0 {
        ...
        puts(b"ps: unable to read /proc/processes\0".as_ptr());
    }
}

unsafe fn display_system_monitor() {
    let mut si = Sysinfo::default();
    if sysinfo(&mut si as *mut _) < 0 {
        puts(b"top: failed to retrieve system metrics\0".as_ptr());
        return;
    }
    ...
    let heap_total_mb = si.bufferram / (1024 * 1024);
    let heap_used_mb = si.sharedram / (1024 * 1024);
    let heap_pct = if heap_total_mb > 0 { (heap_used_mb * 100) / heap_total_mb } else { 0 };

    puts(b"======================================================================\0".as_ptr());
    puts(b"           Rust POSIX OS - System Resource & Process Monitor          \0".as_ptr());
    puts(b"======================================================================\0".as_ptr());
    printf(b"  Uptime:       %d ticks\n\0".as_ptr(), si.uptime as i32);
    printf(b"  Active Tasks: %d processes\n\0".as_ptr(), si.procs as i32);
    printf(
        b"  Physical RAM: [%d%%] %d MB / %d MB used (%d MB free)\n\0".as_ptr(),
        mem_pct as i32, used_mb as i32, total_mb as i32, free_mb as i32
    );
    printf(
        b"  Kernel Heap:  [%d%%] %d MB / %d MB used\n\0".as_ptr(),
        heap_pct as i32, heap_used_mb as i32, heap_total_mb as i32
    );
    puts(b"----------------------------------------------------------------------\0".as_ptr());
    puts(b"  PID    PPID   STATE       FDS    CWD\0".as_ptr());
    puts(b"----------------------------------------------------------------------\0".as_ptr());
    ...
    if fd >= 0 {
        ...
        if n > 0 {
            ...
            let header_end = strchr(ptr, b'\n' as i32);
            if !header_end.is_null() {
                ptr = header_end.add(1);
            }
            while *ptr != 0 {
                let line_end = strchr(ptr, b'\n' as i32);
                if line_end.is_null() {
                    break;
                }
                let len = line_end as usize - ptr as usize;
                let mut line = [0u8; 128];
                let copy_len = len.min(127);
                core::ptr::copy_nonoverlapping(ptr, line.as_mut_ptr(), copy_len);
                line[copy_len] = 0;
...
unsafe fn run_async_demo() {
    puts(b"=== Running POSIX Asynchronous Tasks & Epoll Demo ===".as_ptr());

    let epfd = epoll_create1(0);
    if epfd < 0 {
        print_error(b"epoll_create1\0".as_ptr(), b"epoll\0".as_ptr(), epfd);
        return;
    }
    printf(b"[async-demo] Created epoll instance with fd = %d\n\0".as_ptr(), epfd);

    let mut pipe1 = [0i32; 2];
    let mut pipe2 = [0i32; 2];
    pipe(&mut pipe1);
    pipe(&mut pipe2);
    printf(b"[async-demo] Spawned Async Task Channels: Pipe1 [%d -> %d], Pipe2 [%d -> %d]\n\0".as_ptr(), pipe1[1], pipe1[0], pipe2[1], pipe2[0]);
    ...
    printf(b"[async-demo] epoll_wait returned %d ready event(s)\n\0".as_ptr(), nready);

    for i in 0..nready as usize {
        let ev = &events[i];
        let mut read_buf = [0u8; 128];
        let (fd, tag) = if ev.data == 1001 { (pipe1[0], "Channel 1") } else { (pipe2[0], "Channel 2") };
        let nread = read(fd, read_buf.as_mut_ptr(), read_buf.len() - 1);
        if nread > 0 {
            read_buf[nread as usize] = 0;
            printf(b"[async-demo] Event on %s (data tag: %d): %s\0".as_ptr(), tag.as_ptr(), ev.data as i32, read_buf.as_ptr());
        }
    }
    ...
    close(pipe2[1]);
    close(epfd);
    puts(b"=== Asynchronous Tasks & Epoll Demo Complete ===\n\0".as_ptr());
}
