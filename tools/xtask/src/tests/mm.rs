...
impl MockVmSpace {
    ...
    fn munmap(&mut self, addr: usize, len: usize) -> Result<(), i32> {
        if !addr.is_multiple_of(4096) || len == 0 { return Err(EINVAL); }
        let end = addr + len;
    ...
    fn mprotect(&mut self, addr: usize, len: usize, new_prot: u32) -> Result<(), i32> {
        if !addr.is_multiple_of(4096) || len == 0 { return Err(EINVAL); }
        let end = addr + len;
        if !self.contains_range(addr, end) { return Err(ENOMEM); }
        let flags = self.vmas.iter().find(|v| v.start <= addr && addr < v.end).map(|v| v.flags).unwrap_or(0);
...
fn test_vma_tracking() {
    ...
    vm.insert_vma(0x6000_0000, 0x6000_2000, (PROT_READ | PROT_WRITE) as u32, MAP_ANON);
    ...
    assert_eq!(vm.mprotect(0x6000_1000, 8192, PROT_READ as u32), Err(ENOMEM));
...
fn test_fork_address_space_isolation() {
    ...
    assert_eq!(&parent.pages.get(&0x6000_0000).unwrap()[0..4], &[0x42, 0x43, 0x44, 0x45]);
    assert_eq!(&child.pages.get(&0x6000_0000).unwrap()[0..4], &[0x99, 0x88, 0x77, 0x66]);
...
fn test_mmap_bounds_and_overflow() {
    ...
    let check_mmap_addr = |addr: usize, len: usize| -> Result<usize, i32> {
        ...
        if len > USER_SPACE_END { return Err(ENOMEM); }
        let byte_len = len.div_ceil(PAGE_SIZE).checked_mul(PAGE_SIZE).ok_or(ENOMEM)?;
        let vaddr = if addr == 0 { 0x0000_7000_0000_0000 } else { addr & !0xFFF };
        let end = vaddr.checked_add(byte_len).ok_or(ENOMEM)?;
        if end > USER_SPACE_END { return Err(ENOMEM); }
        Ok(vaddr)
    };
...
fn test_mmap_rollback_on_partial_failure() {
    ...
    impl mm_core::FrameAllocator for MockPmm {
        ...
        fn alloc_frame(&mut self) -> Option<usize> {
            if self.alloc_count >= self.fail_after || self.free_frames == 0 { return None; }
            self.free_frames -= 1;
            self.alloc_count += 1;
            Some(0x1000 + self.alloc_count * mm_core::PAGE_SIZE)
        }
        fn free_frame(&mut self, _phys: usize) { self.free_frames += 1; }
    }
    ...
    impl mm_core::PageMapper<usize> for MockPagingSpace {
        fn map_page(&mut self, virt: usize, phys: usize, _prot: u32) -> Result<(), &'static str> {
            self.map_count += 1;
            if self.fail_at_page == Some(self.map_count) { return Err("OOM"); }
            self.mapped_pages.insert(virt, phys);
            Ok(())
        }
        fn unmap_range(&mut self, start_virt: usize, count: usize) {
            let aligned = start_virt & !0xFFF;
            for i in 0..count { self.mapped_pages.remove(&(aligned + i * mm_core::PAGE_SIZE)); }
        }
    }

    let mut pmm = MockPmm { free_frames: 5, fail_after: 5, alloc_count: 0 };
    let mut vm = MockPagingSpace { mapped_pages: BTreeMap::new(), fail_at_page: None, map_count: 0 };
    let mut mmap_next_vaddr = DEFAULT_MMAP_BASE;

    let res = mm_core::mmap_allocate(0, 10 * mm_core::PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, USER_SPACE_END, pmm.free_frames, &mut mmap_next_vaddr, &mut pmm, &mut vm, |_| {});
    ...
    let mut pmm_fault = MockPmm { free_frames: 10, fail_after: 3, alloc_count: 0 };
    let mut vm_fault = MockPagingSpace { mapped_pages: BTreeMap::new(), fail_at_page: None, map_count: 0 };
    let mut mmap_next_fault = DEFAULT_MMAP_BASE;

    let res_fault = mm_core::mmap_allocate(0, 6 * mm_core::PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, USER_SPACE_END, pmm_fault.free_frames, &mut mmap_next_fault, &mut pmm_fault, &mut vm_fault, |_| {});
    assert_eq!(res_fault, Err(ENOMEM));
    assert_eq!(mmap_next_fault, DEFAULT_MMAP_BASE);
    assert!(vm_fault.mapped_pages.is_empty());

    let mut pmm_pt_fault = MockPmm { free_frames: 10, fail_after: 10, alloc_count: 0 };
    let mut vm_pt_fault = MockPagingSpace { mapped_pages: BTreeMap::new(), fail_at_page: Some(3), map_count: 0 };
    let mut mmap_next_pt = DEFAULT_MMAP_BASE;

    let res_pt = mm_core::mmap_allocate(0, 5 * mm_core::PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, USER_SPACE_END, pmm_pt_fault.free_frames, &mut mmap_next_pt, &mut pmm_pt_fault, &mut vm_pt_fault, |_| {});
    ...
    let res_success = mm_core::mmap_allocate(0, 4 * mm_core::PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, USER_SPACE_END, pmm.free_frames, &mut mmap_next_vaddr, &mut pmm, &mut vm, |_| {});
