...
- L3. No calling into VFS while holding a `Process` lock. `cwd`/`creds` (`VfsCreds`) are passed as arguments (enforced by construction and CI: VFS never acquires `Process` or `PROCESS_TABLE` locks; all syscall handlers including `execve` perform VFS resolution and data loading prior to acquiring `Process` locks).
